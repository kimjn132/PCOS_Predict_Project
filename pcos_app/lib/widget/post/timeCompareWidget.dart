        ...
        builder: (BuildContext context, AsyncSnapshot<Duration> snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data!.inSeconds < 60) {
              return Text('${snapshot.data?.inSeconds}초 전');
            } else if (snapshot.data!.inMinutes < 60) {
              return Text('${snapshot.data?.inMinutes}분 전');
            } else if (snapshot.data!.inHours < 24) {
              return Text('${snapshot.data?.inHours}시간 전');
            } else if (snapshot.data!.inDays < 31) {
              return Text('${snapshot.data?.inDays}일 전');
            } else {
